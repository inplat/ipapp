...
oracle_presence = pytest.mark.skipif(
    not oracle_presence(),
    reason="requires oracle database presence with (user='system', password='11111111', dsn='localhost:1522/XE')",
)
...
@oracle_presence
async def test_invalidate_session(loop, oracle_connection):
...
@oracle_presence
async def test_invalidate_session_with_proc_error(loop, oracle_connection):
    app = BaseApplication(BaseConfig())
    app.add(
        'db',
        Oracle(
