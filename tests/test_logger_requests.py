...
from ipapp.app import Application
from ipapp.logger.adapters.requests import RequestsConfig, RequestsAdapter, CREATE_TABLE_QUERY
...
async def get_requests(postgres_url:str, table_name:str,
                       trace_id:str) -> List[asyncpg.Record]:
    conn = await asyncpg.connect(postgres_url)
    query = ('SELECT * FROM {table_name} '
             'WHERE trace_id=$1 ORDER BY id').format(table_name=table_name)
...
async def test_success(postgres_url:str):
    ...
    cfg = RequestsConfig(dsn=postgres_url, db_table_name=table_name, max_hdrs_length=max_hdrs_length, max_body_length=max_body_length, send_max_count=2)
    ...
    with app.logger.span_new(name='request1', kind=HttpSpan.KIND_SERVER, cls=HttpSpan) as span:
        ...
        with span.new_child(name='request2', kind=HttpSpan.KIND_CLIENT,
                                 cls=HttpSpan) as span2:
            with span2.new_child(name='nonrequest') as span3:
                pass
    await app.stop()
